...
async function load(){refreshing.value=true;try{const bid=Number(route.params.bookingId);order.value=await paymentService.getResult(bid);const suffix=kind.value==='success'?'success':kind.value==='cancelled'?'cancelled':'payment-status';const canonical=`/booking/order/${bid}/${suffix}`;if(route.path!==canonical)await router.replace(canonical)}catch(e:any){error.value=e?.response?.status===403?'你无权查看该订单':e?.response?.data?.detail||'服务器暂时不可用'}finally{loading.value=false;refreshing.value=false}}
function property(){router.push(`/room/${order.value?.snapshot.property_id}`)} function rebook(){router.push(`/booking/${order.value?.snapshot.property_id}/move-in-date`)} function retry(){router.push(`/booking/payment/${route.params.bookingId}/deposit`)}
