...
import { formatPropertyPrice, getCurrencySymbol, resolveCurrency } from '@/utils/currency'
...
function formatRent(p: PropertySearchResult | null): string {
  if (!p) return '价格待确认'
  return formatPropertyPrice(p.price_monthly, p.currency, p.country)
}

function formatMoney(p: PropertySearchResult, amount: number | string): string {
  return formatPropertyPrice(amount, p.currency, p.country)
}
...
function priceDiffText(it: CompareItem): string | null {
  ...
  if (currencies.size !== 1 || !it.property) return null
  ...
  const min = sorted[0]
  const symbol = getCurrencySymbol(it.property.currency, it.property.country)
  if (cur === min) {
...
const specRows: SpecRow[] = [
  ...
  { label: '区域', get: (it) => it.property?.district || '—' },
  {
    label: '押金',
    get: (it) =>
      it.property?.deposit_amount != null
        ? formatMoney(it.property, it.property.deposit_amount)
        : '—',
  },
  {
    label: '服务费',
      ...
      <div v-loading="loading" class="cmp-body">
        ...
        <div v-if="tab === 'overview'" class="cmp-pk">
          ...
          >
            ...
            <p v-if="result.summary" class="cmp-ai-summary">{{ result.summary }}</p>
            <div v-if="result.recommendation" class="cmp-ai-conclusion">
              <span class="cmp-ai-label">推荐结论</span>
...
.cmp-ai-conclusion {
  ...
  margin-top: 12px;
  padding-top: 12px;
  border-top: 1px solid rgb(64 158 255 / 14%);
}
.cmp-ai-label {
  flex: 0 0 auto;
  padding: 2px 7px;
