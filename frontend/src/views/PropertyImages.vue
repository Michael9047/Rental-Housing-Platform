<template>
  <div class="images-page" v-loading="imagesLoading">
    <div class="page-header">
      ...
      <el-button text :icon="ArrowLeft" @click="$router.back()">??</el-button>
    </div>
    ...
    <el-card shadow="never" class="upload-card">
      ...
      >
        ...
        <template #tip>
          <div class="el-upload__tip">
            ?? JPG/PNG/WebP?????? 5MB?? {{ images.length }}/10 ?
          </div>
        </template>
      </el-upload>
      <div v-if="fileList.length > 0" class="upload-actions">
        ...
        <el-button @click="fileList = []">??</el-button>
      </div>
    </el-card>
    ...
    <el-empty v-else-if="!imagesLoading" description="????????" />
  </div>
</template>
...
async function handleUpload() {
  ...
  try {
    ...
    fileList.value = []
    ElMessage.success(`???? ${rawFiles.length} ???`)
  } catch {
