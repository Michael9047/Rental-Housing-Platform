...
export const repairService = {
  ...
  updateStatus(id: number, newStatus: string): Promise<RepairRead> {
    return api.patch(`/repairs/${id}/status`, null, { params: { new_status: newStatus } })
  },

  /** 房东指派维修师傅 */
  assignWorker(id: number, workerId: number): Promise<RepairRead> {
    return api.patch(`/repairs/${id}/assign`, null, { params: { worker_id: workerId } })
  },

  /** 维修师傅开始工作 */
  startWork(id: number): Promise<RepairRead> {
    return api.patch(`/repairs/${id}/start`)
  },

  /** 维修师傅完成工单 */
  completeWork(id: number, workRecord: string): Promise<RepairRead> {
    return api.patch(`/repairs/${id}/complete`, null, { params: { work_record: workRecord } })
  },

  /** 租客取消报修 */
  cancel(id: number): Promise<RepairRead> {
    return api.patch(`/repairs/${id}/cancel`)
  },
}
