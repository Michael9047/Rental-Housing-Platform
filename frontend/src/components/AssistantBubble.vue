           ...
           <div class="ab-chat-wrap">
            ...
            <div ref="listRef" class="ab-messages" @click="onMessagesClick">
              <div v-for="(msg, i) in messages" :key="i" class="ab-msg-block">
                ...
                <!-- 房源介绍横条：显示全部匹配项，支持触控/触摸板滑动和左右按钮 -->
                <div v-if="msg.recommendations && msg.recommendations.length" class="ab-recs-shell">
                  <div class="ab-recs-head">
                    <div>
                      <strong>匹配房源 {{ msg.recommendations.length }} 套</strong>
                      <span>横向滑动查看更多</span>
                    </div>
                    <div class="ab-recs-nav">
                      <button aria-label="向左查看更多房源" @click="scrollRecommendations($event, -1)">‹</button>
                      <button aria-label="向右查看更多房源" @click="scrollRecommendations($event, 1)">›</button>
                    </div>
                  </div>
                  <div class="ab-recs">
                    <div
                    ...
                    >
                      <div class="ab-rec-img">
                        ...
                        <span class="ab-rec-district">{{ rec.property.district }}</span>
                      ...
                      <div class="ab-rec-info">
                        ...
                        <div class="ab-rec-price">¥{{ formatPrice(rec.property.price_monthly) }}<span>/月</span></div>
                        <div class="ab-rec-specs">
                          <span>{{ rec.property.bedrooms }}室{{ rec.property.bathrooms }}卫</span>
                          <span>{{ propertyTypeLabel(rec.property.property_type) }}</span>
                          <span v-if="rec.property.area_sqm">{{ rec.property.area_sqm }}㎡</span>
                          <span>{{ leaseLabel(rec.property) }}</span>
                        </div>
                        <p class="ab-rec-desc">{{ propertyIntroduction(rec.property) }}</p>
                        <div v-if="rec.match_reason" class="ab-rec-reason" :title="rec.match_reason">
                          推荐理由：{{ rec.match_reason }}
                        </div>
                        <div v-if="amenityHighlights(rec).length" class="ab-rec-tags">
                          <span v-for="item in amenityHighlights(rec)" :key="item">{{ item }}</span>
                        </div>
                        <div class="ab-rec-nearby" :class="{ muted: nearbyHighlights(rec).length === 0 }">
                          <strong>周边</strong>
                          <template v-if="nearbyHighlights(rec).length">
                            <span v-for="item in nearbyHighlights(rec)" :key="item">{{ item }}</span>
                          </template>
                          <span v-else>真实周边数据待补充</span>
                        </div>
                        <div v-if="commuteHighlight(rec)" class="ab-rec-commute">{{ commuteHighlight(rec) }}</div>
                        <div class="ab-rec-address" :title="rec.property.address">📍 {{ rec.property.address }}</div>
                        <div class="ab-rec-acts">
                          <button class="ab-rec-detail" @click="goLink(`/property/${rec.property_id}`)">查看详情</button>
                          <el-tooltip
...
/** 视频中的四组找房条件；当前后端未返回 elicit 时由前端直接展示。 */
const findHouseElicit: AgentElicit = {
  allow_custom: true,
  groups: [
    {
      field: 'district',
      ...
      options: [
        { label: '苏州工业园区', value: '苏州工业园区' },
        { label: '苏州姑苏区', value: '苏州姑苏区' },
        { label: '上海', value: '上海' },
        { label: '北京', value: '北京' },
        { label: '深圳', value: '深圳' },
        { label: '不限', value: '__any__' },
      ],
    },
    {
      field: 'price_max',
      ...
      options: [
        ...
        { label: '5000-8000', value: '8000' },
        { label: '8000 以上', value: '20000' },
        { label: '不限', value: '__any__' },
      ],
    },
    {
...
function submitElicitPanel() {
  ...
  applyElicitToFilters(slotAnswers)
  send(undefined, displayParts.join('、') || '不限', slotAnswers)
}

/** 把视频中的条件选择同步到当前后端已经支持的结构化 filters。 */
function applyElicitToFilters(slotAnswers: Record<string, string>) {
  const district = slotAnswers.district
  filters.district = district && district !== '__any__' ? district : null

  const price = slotAnswers.price_max
  if (!price || price === '__any__') {
    filters.price_min = null
    filters.price_max = null
  } else {
    const max = Number(price)
    filters.price_max = max
    filters.price_min = max === 3000 ? 2000 : max === 5000 ? 3000 : max === 8000 ? 5000 : max === 20000 ? 8000 : null
  }

  const bedrooms = slotAnswers.bedrooms
  filters.bedrooms = bedrooms && bedrooms !== '__any__' ? Number(bedrooms) : null

  const propertyType = slotAnswers.property_type
  filters.property_type = propertyType && propertyType !== '__any__'
    ? propertyType as AgentFilters['property_type']
    : null
}
...
// 首页等页面可以直接唤起 AI 管家并发送一句话，切页后也不会丢失。
watch(() => agentChat.pendingQuery, async () => {
  const query = agentChat.consumeQuery()
  if (!query) return
  open.value = true
  view.value = 'chat'
  try {
    await agentChat.ensureSession()
    if (/我想租房|帮我推荐/.test(query)) {
      await showFindHouseElicit(query)
    } else {
      await send(query)
    }
  } catch {
...
function leaseLabel(property: PropertySearchResult): string {
  const min = property.min_lease_months
  const max = property.max_lease_months
  if (min && max && min !== max) return `${min}-${max}个月`
  if (min) return `${min}个月起租`
  return '租期可咨询'
}

function propertyIntroduction(property: PropertySearchResult): string {
  ...
  return `位于${property.district}的${propertyTypeLabel(property.property_type)}，具体配置可进入详情页查看。`
}

function amenityHighlights(rec: AgentRecommendation): string[] {
  return (rec.property.amenities || []).filter(Boolean).slice(0, 4)
}
...
function nearbyHighlights(rec: AgentRecommendation): string[] {
  const facts = rec.facts
  if (!facts?.data_completeness.poi_cache_available) return []
  const poi = facts.poi
  const values: string[] = []
  if (poi.nearest_transit_m != null) values.push(`交通 ${formatDistance(poi.nearest_transit_m)}`)
  if (poi.nearest_supermarket_m != null) values.push(`超市 ${formatDistance(poi.nearest_supermarket_m)}`)
  if (poi.nearest_gym_m != null) values.push(`健身 ${formatDistance(poi.nearest_gym_m)}`)
  if (poi.nearest_medical_m != null) values.push(`医疗 ${formatDistance(poi.nearest_medical_m)}`)
  return values.slice(0, 4)
}

function commuteHighlight(rec: AgentRecommendation): string | null {
  const commute = rec.facts?.commute
  if (!commute) return null
  const source = commute.source === 'haversine_fallback' ? '估算' : '路线数据'
  return `🚗 驾车约 ${commute.drive_min} 分钟 · 🚌 公交约 ${commute.transit_min} 分钟（${source}）`
}

function scrollRecommendations(event: MouseEvent, direction: -1 | 1) {
  const button = event.currentTarget as HTMLElement | null
  const row = button?.closest('.ab-recs-shell')?.querySelector<HTMLElement>('.ab-recs')
  if (!row) return
  row.scrollBy({ left: direction * Math.max(260, row.clientWidth * 0.78), behavior: 'smooth' })
}

function mergeRecommendations(
  topPicks: AgentRecommendation[] | undefined,
  allRecommendations: AgentRecommendation[] | undefined,
): AgentRecommendation[] {
  const merged: AgentRecommendation[] = []
  const seen = new Set<number>()
  for (const recommendation of [...(topPicks || []), ...(allRecommendations || [])]) {
    if (seen.has(recommendation.property_id)) continue
    seen.add(recommendation.property_id)
...
async function send(preset?: string, display?: string, slotAnswers?: Record<string, string>) {
  ...
  if (!slotAnswers && /^(我想租房|帮我推荐|帮我找房)/.test(text)) {
    if (!preset) inputText.value = ''
    await showFindHouseElicit(display ?? text)
    return
  }
  ...
  try {
    ...
    messages.value.push({
      id: resp.message_id ?? undefined,
      role: 'assistant',
      content: resp.reply,
      topPicks: resp.top_picks,
      allRecommendations: resp.recommendations,
      recommendations: resp.intent === 'recommend'
        ? mergeRecommendations(resp.top_picks, resp.recommendations)
        : undefined,
      aiAvailable: resp.ai_available,
      quickReplies: resp.quick_replies,
      links: resp.links,
      elicit: resp.elicit ?? undefined,
      feedback: null,
      intent: resp.intent,
      thinkingSteps: resp.thinking_steps,
    })
    agentChat.aiAvailable = resp.ai_available
...
async function triggerFindHouse() {
  await showFindHouseElicit('我想租房，帮我推荐一下')
}

async function showFindHouseElicit(display: string) {
  if (sending.value || sessionId.value === null) return
  messages.value.push({ role: 'user', content: display })
  messages.value.push({
    role: 'assistant',
    content: '还差这几项，选一下吧（也可以直接跳过）：',
    intent: 'elicit',
    elicit: findHouseElicit,
  })
  await scrollToBottom()
}
