...
class UnitTypeService:
    ...
    async def create(self, data: UnitTypeCreate) -> UnitType:
        ...
        ut = UnitType(
            ...
            property_type=getattr(data, 'property_type', None),
            ...
            lease_start_date=getattr(data, 'lease_start_date', None),
            lease_end=data.lease_end,
            lease_end_date=getattr(data, 'lease_end_date', None),
            rental_requirements=getattr(data, 'rental_requirements', None),
            currency=data.currency, special_offer=data.special_offer,
            floor_pricing=getattr(data, 'floor_pricing', None),
            amenities=getattr(data, 'amenities', None),
            description=getattr(data, 'description', None),
            available_from=data.available_from, min_stay_months=data.min_stay_months,
            total_count=getattr(data, 'total_count', 1),
            available_count=getattr(data, 'available_count', 1),
            has_vacancy=getattr(data, 'has_vacancy', True),
            status=data.status,
        )
        self.session.add(ut)
