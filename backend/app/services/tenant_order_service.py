...
class TenantOrderService:
    ...
    async def detail_for_tenant(self, booking_id: int, tenant_user_id: int) -> TenantOrderDetail:
        ...
        if not unit_type or not tenant:
    ...
    def _workflow(booking: Booking, payment: Payment | None, contract: Contract | None) -> dict:
        ...
        if contract and contract.status == "generated":
            return {"current_step": "awaiting_contract", "title": "等待BM发送合同", "can_view_contract": False, "can_sign_contract": False}
        if contract and contract.status in {"sent", "awaiting_signature"}:
