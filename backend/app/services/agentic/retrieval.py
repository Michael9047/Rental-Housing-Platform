...
def _source_metadata(item: dict[str, Any]) -> dict[str, Any]:
    ...
    legacy = item.get("_source_kind") == "legacy_property"
    return {
        "property": "properties" if legacy else "unit_types",
        "institute": "properties.flattened" if legacy else "institutes",
        "inventory": "properties.status" if legacy else "rooms",
        "semantic": (
            ("properties.embedding" if legacy else "unit_types.embedding")
            if item.get("embedding_score") is not None else "missing"
        ),
        "commute": commute_source,
        "poi": "property_pois" if poi_distances else "missing",
        "missing_fields": [
