...
POI 数据挂在 Room 上（PropertyPOI.property_id -> Room.id），本模块负责经
Room.unit_type_id 把 POI 聚合回 unit_type 粒度。同一 institute 下坐标一致，
故取该户型任一有 POI 的房间即可代表其周边。
"""
...
async def load_unit_type_poi(
    ...
    经 Room 桥接：PropertyPOI.property_id -> Room.id -> Room.unit_type_id。
    ...
    try:
        stmt = (
            select(Room.unit_type_id, PropertyPOI.poi_data)
            .join(PropertyPOI, PropertyPOI.property_id == Room.id)
            .where(Room.unit_type_id.in_(unit_type_ids))
        )
    ...
    # 同一 unit_type 可能有多个房间/POI 记录，取第一个非空的即可（同 institute 坐标一致）
