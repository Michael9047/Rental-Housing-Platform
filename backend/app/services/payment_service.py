...
class PaymentOrderService:
    ...
    def _price_snapshot(booking: Booking, contract: Contract, property_obj: Property, tenant_name: str) -> tuple[dict, dict]:
        ...
        ut = property_obj  # 三层架构：property_obj 即 UnitType
        inst = getattr(ut, 'institute', None) if ut else None
        snapshot = {
            "order_number": str(booking.id), "property_id": booking.unit_type_id,
            "property_name": ut.name or f"Room #{ut.id}", "property_address": getattr(inst, 'address', '') if inst else '',
    ...
    async def create(self, booking_id: int, user_id: int, idempotency_key: str, tenant_name: str, payment_method: PaymentMethod = PaymentMethod.CARD_CHECKOUT) -> Payment:
        ...
        property_obj = await self.session.scalar(
            select(Property).where(Property.id == booking.unit_type_id).options(_sil(Property.institute))
        )
    ...
    async def process_webhook(self, payload: bytes, signature: str) -> Payment:
        ...
        if event.get("status") == "succeeded":
            if target == BookingStatus.payment_review:
                ...
                return payment
            property_obj = await self.session.scalar(select(Property).where(Property.id == booking.unit_type_id).with_for_update())
