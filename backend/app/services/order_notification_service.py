...
class OrderNotificationService:
    ...
    async def enqueue(self, event_type: str, booking: Booking, *, payment: Payment | None = None, contract: Contract | None = None, discriminator: str = "1") -> NotificationOutbox | None:
        ...
        user = await self.session.get(User, booking.tenant_id); property_obj = await self.session.get(Property, booking.unit_type_id)
        ...
        property_name = property_obj.name or getattr(property_obj, 'room_number', None) or f"Room #{property_obj.id}"
        property_address = getattr(getattr(property_obj, 'institute', None), 'address', '') or ''
    ...
    async def enqueue_landlord_booking_confirmed(
        ...
        *,
        ...
        if not property_obj:
            return None
        landlord = await self.session.get(User, getattr(getattr(property_obj, 'institute', None), 'bm_id', None))
        if not landlord:
            return None
        if contract is None:
        ...
        property_name = property_obj.name or f"Room #{property_obj.id}"
        property_address = getattr(getattr(property_obj, 'institute', None), 'address', '') or ''
